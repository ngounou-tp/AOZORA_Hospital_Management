...
class IndustriLayoute extends StatefulWidget {
  IndustriLayoute({Key? key}) : super(key: key);

  @override
  State<IndustriLayoute> createState() => _IndustriLayouteState();
}

class _IndustriLayouteState extends State<IndustriLayoute> {
...
class DataTableExampleState extends State<DataTableExample> {
  ...
  List<Item> _generateItems() {
    return List.generate(10, (int index) {
      return Item(
        nom: "Immobilier",
        nomcomplet: "Activite immobilier",
        isSelected: false,
      );
    });
  }
  ...
  DataRow _createRow(Item item) {
    return DataRow(
      ...
      cells: [
        DataCell(
          field == 1 ? Text(item.nom) : TextField(),
        ),
        DataCell(
          field == 1 ? Text(item.nomcomplet) : TextField(),
          placeholder: false,
          onTap: () {
            print('onTap');
          },
